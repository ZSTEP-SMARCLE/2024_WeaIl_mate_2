...
def chat(inputs):
    ...
    nb = pm.execute_notebook(
        ...
        parameters={"input_string": inputs},  # 변수 전달
    ...
    with col0:
        with st.popover(""):
            ...
            if st.button("시각장애인을 위한 유튜브 추천", use_container_width=1):
                chat("시각장애인을 위한 유튜브 추천 영상을 추천해서 링크를 같이 첨부해줘")
            if st.button("지적장애인을 위한 엔터테인먼트 영상", use_container_width=1):
                chat("지적장애인을 위한 엔터테인먼트 영상을 추천해줄 수 있나요? 유튜브 링크를 같이 첨부해줘요")
            if st.button("지체장애인을 위한 운동 영상", use_container_width=1):
                chat("지체장애인을 위한 운동 영상을 추천해서 링크를 같이 첨부해줘")
    with col1:
